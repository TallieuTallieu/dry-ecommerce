...
| `PaymentInterface`     | `pay()` now returns a `PaymentOutcome` (`PaymentRedirect`, `PaymentSettled`, `PaymentRefused`) instead of redirecting or dispatching itself | `provider(): string`; plus the opt-in `PaymentGatewayInterface` (`reportOf()`) beside it: the [gateway harness](payment.md#writing-a-gateway) a real provider implements instead of inventing its own webhook plumbing |
